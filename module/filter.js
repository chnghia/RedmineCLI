...
var resolveIdsForNames = function (options) {
  if (options.priority)
    options.priority = redmine.getPriorityIdByName(options.priority);
  if (options.status)
    options.status = redmine.getStatusIdByName(options.status);
  if (options.tracker)
    options.tracker = redmine.getTrackerIdByName(options.tracker);
};
